...
func Test5BasicPersistence(t *testing.T) {
	tc := setup(t, "basicpersistence", 1, 3, false) // 1 group, 3 servers, reliable
	defer tc.cleanup() 								// defer the cleanup

	fmt.Printf("Test: Basic Persistence ...\n")

	tc.join(0) 										// make group 100 available

	ck := tc.clerk() 								// instantiate client

	ck.Append("a", "x") 							// append "x" to data["a"]
	ck.Append("a", "y") 							// append "y" to data["a"]
	if ck.Get("a") != "xy" { 						// if get("a") does not yield "xy"
		t.Fatalf("wrong value") 					//    fatal error
	}

	// kill servers with disk intact
	...
	// check that client requests are not executed
...
func Test5OneRestart(t *testing.T) {
		tc := setup(t, "onerestart", 1, 3, false)		// 1 group, 3 servers, reliable
		defer tc.cleanup() 								// defer cleanup

		fmt.Printf("Test: One server restarts ...\n")

		tc.join(0) 										// make group 100 available
		ck := tc.clerk() 								// create client
		g0 := tc.groups[0] 								// get group 100
...
func Test5OneLostOneDown(t *testing.T) {
	...
	tc.kill1(0, 0, false) // kills 0, but saves disk
	...
	for i := 1; i < len(g0.servers); i++ {
		...
		tc.kill1(0, i, true) // kills 1, kills disk
		time.Sleep(1 * time.Second)
...
func Test5Simultaneous(t *testing.T) {
	tc := setup(t, "simultaneous", 1, 3, true) 	// one group, three servers, unreliable
	defer tc.cleanup() 							// defer cleanup()

	fmt.Printf("Test: Simultaneous Append and Crash ...\n")

	tc.join(0) 									// make group 100 available
	ck := tc.clerk()							// create a client

	k1 := randstring(10) 						// randomly generate key 1
	ck.Put(k1, "") 								// put key 1 in the database

	ch := make(chan int) 						// create an integer channel

	ff := func(x int) { 						// not sure what this function is doing...
		ret := -1
		defer func() { ch <- ret }()
		myck := tc.clerk()
	...
	counts := []int{0}  // {[]int, 1, 1} slice with counts[0] = 0				
...
func Test5RejoinMix1(t *testing.T) {
	...
	fmt.Printf("\nKILLING SRV-1 - Lost Disk...\n")
	tc.kill1(0, 1, true)
	fmt.Printf("\nKILLING SRV-2 - Lost Disk...\n")
	tc.kill1(0, 2, true)

	fmt.Printf("\nKILLING SRV-3 - simple reboot...\n")
	tc.kill1(0, 3, false)
	fmt.Printf("\nKILLING SRV-4 - simple reboot...\n")
	tc.kill1(0, 4, false)
