...
func check(t *testing.T, groups []int64, ck *Clerk) {
	...
	fmt.Print("Results of check query:")
	fmt.Println(c)
	fmt.Print("Expected groups: ")
	fmt.Println(groups)
	fmt.Println()
...
func TestBasic(t *testing.T) {
	...
	// 3 element slice of pointers to SM servers
	var sma []*ShardMaster = make([]*ShardMaster, nservers) 
	
	// 3 element slice of strings
	var kvh []string = make([]string, nservers)
	
	// cleanup on exit form TestBasic
	defer cleanup(sma)

	// construct the ports for the 3 SM servers
	for i := 0; i < nservers; i++ {
		kvh[i] = port("basic", i)
	}

	// instantiate each SM server with a unique port
	for i := 0; i < nservers; i++ {
		sma[i] = StartServer(kvh, i)
	}

	// create clerk that can reference all 3 SM servers
	ck := MakeClerk(kvh) 

	// cka is a 3 element array of pointer to clerks
	...
	// cka[0] = MakeClerk([Server 0])
	// cka[1] = MakeClerk([Server 1])
	// cka[2] = MakeClerk([Server 3])

	// then these clerks only know about 1 server?

	fmt.Printf("Test: Basic leave/join ...\n")

	// cfa is a 6 element slice of Config structs
	cfa := make([]Config, 6)
	
	// cfa[0] = ck.Query(-1)
	// Note ck is the clerk that knows all three SM servers
	cfa[0] = ck.Query(-1)
