...
import "strings" // for Fields()
import "unicode" // for IsLetter()
import "strconv" // for Itoa()
...
func Map(value string) *list.List {
    // parse the string into word tokens
    f := func(c rune) bool {
        return !unicode.IsLetter(c)
    }
    words := strings.FieldsFunc(value, f)

    // populate keyValues list with {<word>, "1"} for each word in words 
    keyValues := list.New()
    for _, word := range words {
        keyValues.PushBack(mapreduce.KeyValue{word, "1"})
    }
